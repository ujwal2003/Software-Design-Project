...
<div class={heightStyle}>
	<main class={heightStyle2}>
		<aside class={backroundStyle}>
			<nav class={verticalStyle}>
				{#each sidebarLinks as sideLink}
					<a href={sideLink.link} class="text-[#CBD5E1]"> {sideLink.name} </a>
				{/each}
			</nav>
		</aside>
	</main>
